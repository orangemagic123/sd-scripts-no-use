...
class NetworkEMA:
    def __init__(self, parameters, decay: float, offload_to_cpu: bool = False):
        self.decay = decay
    ...
    def update(self, parameters):
        for shadow_param, param in zip(self.shadow_params, parameters):
            src_param = param.detach()
            if src_param.device != shadow_param.device or src_param.dtype != shadow_param.dtype:
                src_param = src_param.to(device=shadow_param.device, dtype=shadow_param.dtype)
            shadow_param.lerp_(src_param, 1.0 - self.decay)
    ...
    def copy_to(self, parameters):
        for shadow_param, param in zip(self.shadow_params, parameters):
            src_param = shadow_param
            if src_param.device != param.device or src_param.dtype != param.dtype:
                src_param = src_param.to(device=param.device, dtype=param.dtype)
...
def train(args):
    ...
    if train_dit and args.ema_decay is not None:
        ema_model = NetworkEMA(
            list(accelerator.unwrap_model(dit).parameters()),
            args.ema_decay,
            offload_to_cpu=args.ema_cpu,
        )
        logger.info(f"enable EMA for DiT weights with decay={args.ema_decay}, ema_cpu={args.ema_cpu}")
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--ema_cpu",
        action="store_true",
        help="Store EMA shadow weights on CPU to reduce VRAM usage (may slow training) / EMAのshadow重みをCPUに保持してVRAM使用量を減らす（学習速度が低下する可能性あり）",
    )
    parser.add_argument(
