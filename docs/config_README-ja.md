...
* `caption_mode`
    * `caption` は通常のcaptionファイルのみを使います。`mixed` は通常のcaptionファイルをタグcaptionとして使い、さらに `image.txt` に対する `image_nl.txt` のように拡張子の前へ `_nl` を付けた自然言語captionファイルも読み込みます。

* `mixed_weights`
    * `caption_mode = "mixed"` のときに使います。`tags`、`nl`、`tags_nl`、`nl_tags` を指定した重みで選択します。自然言語部分はtag dropoutの影響を受けず、`keep_tokens_separator` より前のタグは常に先頭に残ります。

* `protected_tags_file`
    * 1行に1タグを書いたテキストファイルを指定します。このファイルにあるタグは `caption_tag_dropout_rate` の対象外になりますが、caption のシャッフル対象には残ります。大文字・小文字は区別されません。
...
### `protected_tags_file` の例

`protected_tags.txt` の内容が以下の場合:
...
これらのタグは他のタグと一緒にシャッフルされますが、`caption_tag_dropout_rate` では削除されません。
